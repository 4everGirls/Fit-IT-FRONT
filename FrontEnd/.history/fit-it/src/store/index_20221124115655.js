...
export default new Vuex.Store({
  ...
  actions: {
      ...
      return new Promise((response, reject) => {
        ...
        axios({
          ...
          .then(() => {
            ...
            router.push("/detailChallenge/${payload.challengeNo}"); //나중에 ${challengeNO}로 보내기
          })
          .catch((err) => {
            console.log(err);
            reject(err);
    ...
    inDetailGetMissionList({ commit }, challengeNo) {
      ...
      const API_URL = `${REST_API}/challengeApi/detailMission/${challengeNo.challengeNo}`;
    ...
    getMyJoinChallenges({ commit }, userNo) {
      ...
      axios({
        ...
        .then((res) => {
          ...
          console.log("adfadf");
          console.log(myjoinchallenges);
          //myJoinChalleng에다가 참여중인지 완료인지 정보 넣기 -
          
          for(var i in myjoinchallenges){
            const API_URL = `${REST_API}/challengeApi/isCompleteChallenge/${myjoinchallenges[i].challengeNo}/${myjoinchallenges[i].userNo}`;
            let isCompleteVar = "N";         
            axios({
              url: API_URL,
              method: "GET",
              ...
              myjoinchallenges[i].isComplete = isCompleteVar;
            });
            console.log(myjoinchallenges[i]);
          }
    ...
    isJoinOrNotLetCheck({ commit }, joinInfo) {
      console.log(joinInfo.challenge.participantInfo);
      console.log(joinInfo.challenge.participantInfo[0]);
      let temp = 0;
      for (let i=0; i< joinInfo.challenge.participantInfo.length; i++) {
        console.log(joinInfo.challenge.participantInfo[i]);
        console.log(joinInfo.userNo);
        // 위와 같은 동작을 하는 for / in 문
        if (joinInfo.challenge.participantInfo[i] === joinInfo.userNo) {
          temp = 1;
        }
      }
      console.log("tragtfaf");
      console.log(temp);
      commit("IS_JOIN", temp);
      ...
      let temp = 0;
      for (var i in joinInfo.challenge.participantInfo) {
        // 위와 같은 동작을 하는 for / in 문
    ...
    completeChallenge({ commit }, payload){
      const API_URL = `${REST_API}/challengeApi/completeMission/${payload.challengeNo}/${payload.userNo}`;
      commit;
      ...
      axios({
        ...
        alert("챌린지 완료되었습니다. 축하합니다 짝짝짝");
      });
    },
    isCompleteChallenge({ commit }, completeInfo) {
      //현재 참여중인지 아닌지 N Y로 RETURN
      
      const API_URL = `${REST_API}/challengeApi/isCompleteChallenge/${completeInfo.challengeNo}/${completeInfo.userNo}`;
