...
export default new Vuex.Store({
  ...
  state: {
    ...
    challenge: {},    // 챌린지 하나
    challenges: [],   // DB에서 가져온 챌린지 정보(GET)
    reviews: [],      // 챌린지에 달린 리뷰
  ...
  actions: {
    ...
    makeChallenge({commit}, challenge) {
      ...
      axios({
        ...
        .then((res) => {
          ...
          commit("setchallengeNo")
        }).then(()=>{
          const API_URL2 = `${REST_API}/challengeApi/insertMission/${res.data}`; 
          axios({
            url: API_URL2,
            method: "POST",
            data: challenge,
    ...
    makeMission({commit}, challenge) {
      const API_URL = `${REST_API}/challengeApi/insertMission/{challengeNo}`; 
      axios({
        url: API_URL,
        method: "POST",
        data: challenge,
