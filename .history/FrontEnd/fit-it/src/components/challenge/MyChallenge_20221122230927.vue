...
<script>
import { mapState, loginUser } from "vuex";

export default {
  name: "MyChallenge",
  computed: {
    ...mapState(["myjoinchallenges"]),
    isNotEmpty() {
      console.log(this.myjoinchallenges.length);
  ...
  created() {
    this.$store.dispatch("getMyJoinChallenges", loginUser.userNo);
  }
};
</script>
