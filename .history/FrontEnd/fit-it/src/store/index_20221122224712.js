...
export default new Vuex.Store({
  ...
  mutations: {
    ...
    POST_JOIN_INFO(state, joinParticipantInfo) {
      state.joinParticipantInfo = joinParticipantInfo;
    }
  ...
  actions: {
    ...
    inDetailGetMissionList({ commit }, challengeNo) {
      const API_URL = `${REST_API}/challengeApi/detailMission/PathVariable`;
    ...
    joinChallenge({state, commit}, joinInfo) {
      ...
      const API_URL = `${REST_API}/challengeApi/joinChallenge/${joinInfo.challengeNo}/${joinInfo.challengeNo}`;
      commit("POST_JOIN_INFO", joinInfo);
      console.log(state.joinParticipantInfo);
      axios({
        url: API_URL,
        method: "GET",
