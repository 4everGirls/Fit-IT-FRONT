...
export default new Vuex.Store({
  ...
  actions: {
    ...
    getChallengesAndMissions({ commit }) {
      ...
      axios({
        ...
        Promise.all(challenges.map((challenge) => {
          return axios.get(`${REST_API}/challengeApi/detailMission/${challenge.challengeNo}`)
        })).then((missionLists)=>{
    ...
    //조인했는지 안했는지 체크하자
    isJoinOrNotLetCheck({state,commit},joinInfo){
      const API_URL1 = `${REST_API}/challengeApi/isJoinChallenge/${joinInfo.challengeNo}/${userNo}`;
      if
    },
